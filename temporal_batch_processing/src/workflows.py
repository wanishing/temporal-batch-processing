...
with workflow.unsafe.imports_passed_through():
    ...
    from temporal_batch_processing.src.models.partition_workflow_input import (
        PartitionWorkflowInput,
    )
    from temporal_batch_processing.src.models.partition_workflow_progress import (
        PartitionWorkflowProgress,
    )
    from temporal_batch_processing.src.activities import (
        list_files_activity,
    ...
    from temporal_batch_processing.src.models.batch_processing_request import (
...
class BatchWorkflow:
    ...
    async def process_batch(
        ...
        await workflow.execute_activity(
            enrich_orders_activity, args=[orders], start_to_close_timeout=TIMEOUT
        )
        ...
        if info.parent and info.parent.workflow_id:
            ...
            await parent.signal(
                PartitionWorkflow.report_completion,
...
class PartitionWorkflow:
    ...
    async def process_partition(self, partition_input: PartitionWorkflowInput) -> None:
        ...
        while True:
            await workflow.wait_condition(
                lambda: len(self._in_process_records)
                < partition_input.sliding_window_size
            )
            ...
            if current_offset >= partition_input.end_offset:
                await workflow.wait_condition(
                    lambda: len(self._in_process_records) == 0
                )
                await self._report_progress(workflow.info())
                break

            batch_end = min(
                current_offset + partition_input.batch_size, partition_input.end_offset
            )
    ...
    async def _report_progress(self, info):
        if info.parent and info.parent.workflow_id:
            parent = workflow.get_external_workflow_handle_for(
                workflow=MainWorkflow.run, workflow_id=info.parent.workflow_id
            )
            await parent.signal(
                MainWorkflow.report_progress,
                PartitionWorkflowProgress(
                    child_id=info.workflow_id, completed=self._progress
                ),
            )

    @workflow.signal
    async def report_completion(self, batch_completion: BatchCompletion) -> None:
        reported_records = set(
            range(
                batch_completion.start, batch_completion.start + batch_completion.limit
            )
        )
...
class MainWorkflow:
    ...
    async def _create_partitions(
        ...
        input_files: List[str] = await workflow.execute_activity(
            list_files_activity,
            args=[batch_request.files_path],
            start_to_close_timeout=TIMEOUT,
        )

        files_metadata: List[FileMetadata] = await workflow.execute_activity(
            fetch_files_metadata_activity,
            args=[batch_request.files_path, input_files],
            start_to_close_timeout=TIMEOUT,
        )
    ...
    async def report_progress(self, progress: PartitionWorkflowProgress) -> None:
        self._completion_by_partition[progress.child_id]["completed"] = (
            progress.completed
        )
    ...
    def get_progress(self) -> dict:
        ...
        completed = sum(
            [v["completed"] for v in self._completion_by_partition.values()]
        )
