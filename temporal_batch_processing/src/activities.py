...
from temporal_batch_processing.src.commands.create_partitions_command import CreatePartitionsCommand
from temporal_batch_processing.src.commands.enrich_order_command import EnrichOrdersCommand
from temporal_batch_processing.src.commands.fetch_files_metadata_command import FetchFilesMetadataCommand
...
@activity.defn
async def fetch_files_metadata_activity(batches_path: FilePath, batches_to_process: List[str]) -> List[
    FileMetadata]:
    return FetchFilesMetadataCommand.build(get_config()).run(batches_path, batches_to_process)


@activity.defn
async def create_partitions_activity(files_metadata: List[FileMetadata], partition_size: int) -> \
        List[PartitionInfo]:
...
@activity.defn
async def read_orders_activity(file_info: FileMetadata,
                               start_offset: int,
