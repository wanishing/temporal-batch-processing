...
def random_timestamps(count: int) -> List[str]:
    ...
    return [(base_date + timedelta(seconds=s)).strftime('%Y-%m-%d %H:%M:%S') for s in random_seconds]
...
def run():
    ...
    with duckdb.connect() as con:
        ...
        export_to_parquet(con, "SELECT * FROM orders WHERE order_id <= 5", "orders_5.parquet")
        export_to_parquet(con, "SELECT * FROM orders WHERE order_id > 5 AND order_id <= 12", "orders_7.parquet")
        export_to_parquet(con, "SELECT * FROM orders WHERE order_id > 12 AND order_id <= 20", "orders_8.parquet")
...
def run_many_orders():
    ...
    with duckdb.connect() as con:
        ...
        for batch in generate_dynamic_orders(count):
            ...
            logger.info(f"Inserted {total_inserted}/{count} orders ({(total_inserted / count) * 100:.1f}%)")
