...
import grpc
import pytest
from temporal_batch_processing.src.generated_protos.order_pb2_grpc import (
...
REQUEST = BatchProcessingRequest(
    files_path=FilePath(bucket="some-bucket", key="some-key"),
    batching_options=BatchingOptions(
        sliding_window_size=4, partition_size=5, batch_size=2
    ),
)
...
def _given_files_uploaded(s3, path):
    ...
    for parquet_file in resources_dir.glob("*.parquet"):
        ...
        bucket.upload_file(str(parquet_file), s3_key)
